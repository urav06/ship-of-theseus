...
"""
import http.server, json, os, pty, select, signal, subprocess, sys, threading, time
from datetime import date

STATE_DIR = os.path.join(os.path.expanduser(os.environ.get("XDG_STATE_HOME", "~/.local/state")), "claude-system-prompt")
...
version = subprocess.run(["claude", "--version"], capture_output=True, text=True).stdout.strip().split()[0]
...
with open(AGENT_PATH, "w") as f:
    ...
    f.write(f"description: Anthropic's default Claude Code system prompt, captured from {version} on {date.today()}. Same as a plain session.\n")
    f.write("---\n")
    f.write(req["system"][-1].get("text", ""))

print(f"captured: model={req.get('model')} claude={version} system_blocks={len(req['system'])} tools={len(req.get('tools', []))}")
